...
//        } else
    ...
    /**设置状态栏透明 底部虚拟键不透明 */
    public static void setStatusTranslucent(Window window, Activity activity){
...
//            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            int type = MUIImmerseUtils.StatusBarLightMode(activity);
            if (type != 0) {
                MUIImmerseUtils.StatusBarLightMode(activity, type);
            }
        }
    }
    /**设置状态白色 */
    public static void setStatusWhite(Window window, Activity activity){
        window.requestFeature(Window.FEATURE_NO_TITLE);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            // Android 5.0 以上 全透明
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS
