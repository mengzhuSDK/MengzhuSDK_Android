...
import com.mzmedia.pullrefresh.internal.Utils;
import com.mzmedia.pullrefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public void setNoMoreLabel(boolean isShow) {
        getLoadingLayoutProxy().setNoMoreLabel(isShow);
    }
     /**
     * @deprecated You should now call this method on the result of
     * {@link #getLoadingLayoutProxy()}.
    ...
    public void setLabelColor(int colorRes) {
        getLoadingLayoutProxy().setLabelColor(colorRes);
    }

    public void setUpDrawable(Drawable upDrawable) {
        getLoadingLayoutProxy().setUpDrawable(upDrawable);
    }

    public void setDownDrawable(Drawable downDrawable) {
        getLoadingLayoutProxy().setLoadDrawable(downDrawable);
    }
    ...
    protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //if (mState != State.RESET || mRefreshableView instanceof ListView && ((ListView)mRefreshableView).getLastVisiblePosition()==-1){
...
//                修改上拉加载时候显示空布局，但是刷新时候空布局还在问题
                setNoMoreLabel(false);
